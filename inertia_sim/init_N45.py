...
def sum_generator_Sn_NO2(model_data):
    """
    Calculate the sum of generator S_n in the NO_2 price area.
    ...
    Returns:
    float
        The sum of generator S_n in the NO_2 price area.
    """
    no2_area = 'NO_2'
    index_area = model_data['buses'][0].index('Area')
    index_bus_name = model_data['buses'][0].index('name')
    index_Sn = model_data['generators']['GEN'][0].index('S_n')
    index_bus = model_data['generators']['GEN'][0].index('bus')
    area_mapping = {24: 'SE_4', 23: 'SE_3', 22: 'SE_2', 21: 'SE_1', 11: 'NO_1', 12: 'NO_2', 13: 'NO_3',
                    14: 'NO_4', 15: 'NO_5', 31: 'FI'}
    no2_generators_Sn_sum = 0
    for row in model_data['generators']['GEN'][1:]:
        bus_name = row[index_bus]
        for bus in model_data['buses'][1:]:
            if bus[index_bus_name] == bus_name and bus[index_area] == no2_area:
                no2_generators_Sn_sum += row[index_Sn]
                break
    return no2_generators_Sn_sum

if __name__ == '__main__':
    model = model_data.load()
    ps = dps.PowerSystemModel(model=model)

    # Calculate and print the sum of generator S_n in NO_2 price area
    no2_generators_Sn_sum = sum_generator_Sn_NO2(model)
    print(f"Sum of generator S_n in NO_2 price area: {no2_generators_Sn_sum}")
