...
def HVDC_cable_trip(ps,folderandfilename,t=0,t_end=50,t_trip = 17.6,event_flag = True, link_name = 'NO_2-DE'):
    ''''
...
def display_power_flow(ps, model, international_links, fault_bus, PowerExc_by_country):
    """
    Displays the power flow in the Nordic 45 system, with VSC
    """
    ...
    for name, bus, P in zip(ps.loads['Load'].par['name'], ps.loads['Load'].par['bus'], ps.loads['Load'].p(x0, v0).copy()):
        ...
        if link is None: # if name is not in international_links.keys()
