...
def init_n45(fault_bus = '3359',fault_Sn = 1400,fault_P = 1400,kinetic_energy_eps = 300e3):
    ...
    ps = init_n45_with_HVDC_cables(fault_bus, fault_Sn, fault_P, kinetic_energy_eps)
    ps.power_flow()
    ps.init_dyn_sim()
    x0 = ps.x0.copy()
    v0 = ps.v0.copy()
...
def init_n45_pf(fault_bus = '3359',fault_Sn = 1400,fault_P = 1400,kinetic_energy_eps = 300e3, display = False):
    # ------------------------------ Simulation parameters -----------------------------------
    # fault_bus = '7000' #'3359' #7000
    # fault_Sn = 1400 #1400 #1110
    # fault_P = 1400
    ...
    if display:
        ...
        # ------------------------- Transmission lines -------------------------
        for from_bus, to_bus, p_to, p_from in zip( ps.lines['Line'].par['from_bus'], ps.lines['Line'].par['to_bus'],
...
def gen_trip_hallvar(fault_bus = '3359',fault_Sn = 1400,fault_P = 1400,kinetic_energy_eps = 300e3, folderandfilename = 'Base/300MWs',t=0,t_end=50,t_trip = 17.6,event_flag = True,VSC=False):
    ...
    while t < t_end:
        ...
        if(VSC):
            ...
            res['VSC_name'].append(ps.vsc['VSC'].par['name'])
...
def init_VSC(ps):
    ...
    vsc_international_links = {'L5230-1': 'NO_2-DE', 'L5240-2': 'NO_2-GB','L8700-2': 'SE_4-LT', 'L7020-1': 'FI-EE'}
    vsc_power_exchange = {'NO_2-DE': 0, 'NO_2-GB': 0.1, 'SE_4-LT': 0, 'FI-EE': 0.79}
    ...
    for name, power in vsc_power_exchange.items():
        if name == ps.vsc['VSC_SI'].par['name']:    
            ps.vsc['VSC_SI'].set_input('p_ref', power)
        

  
        
def test_init_VSC(fault_bus = '3359',fault_Sn = 1400,fault_P = 1400,kinetic_energy_eps = 300e3, folderandfilename = 'Base/300MWs',t=0,t_end=50, VSC = True):
